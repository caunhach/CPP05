...
void testInternDefaultConstructor() {
    std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
    ...
    std::string expectedOutput = "Intern default constructor called\n";
    std::string actualOutput = buffer.str();

    assert(actualOutput == expectedOutput);
	std::cout << GREEN << "   " << "\u2714" << " Intern Default Constructor Call's Message correct\n" << RESET;

    delete a;
}

void testInternCopyConstructor() {
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());

    Intern *a = new Intern();
	Intern *e = new Intern(*a);

    std::cout.rdbuf(oldCout);

	std::string expectedOutput = "Intern default constructor called\nIntern Copy constructor called\nIntern Assignment operator called\n";
    std::string actualOutput = buffer.str();

    assert(actualOutput == expectedOutput);
	std::cout << GREEN << "   " << "\u2714" << " Intern Copy Constructor Call's Message correct\n" << RESET;

    delete a;
	delete e;
}

void testInternAssignmentOperator() {
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
	...
	*f = *b;
    ...
    assert(actualOutput == expectedOutput);
	std::cout << GREEN << "   " << "\u2714" << " Intern Assignment Operator Call's Message correct\n" << RESET;

    delete b;
	delete f;
}

void testInternmakeformPresidentialPardonForm()
{
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
	...
	try {
		...
		delete(a.makeForm("presidential pardon", "xxx"));
		...
		assert(actualOutput == expectedOutput);
		std::cout << GREEN << "   " << "\u2714" << " Test passed for Intern makeform PresidentialPardonForm\n" << RESET;
	} catch (std::exception &e) {
		assert(false && "should not thrown an exception");
	}
}

void testInternmakeformRobotomyRequestForm()
{
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
	...
	try {
		...
		delete(a.makeForm("robotomy request", "xxx"));
		...
		assert(actualOutput == expectedOutput);
		std::cout << GREEN << "   " << "\u2714" << " Test passed for Intern makeform RobotomyRequestForm\n" << RESET;
	} catch (std::exception &e) {
		assert(false && "should not thrown an exception");
	}
}

void testInternmakeformShrubberyCreationForm()
{
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
	...
	try {
		...
		delete(a.makeForm("shrubbery creation", "xxx"));
		...
		assert(actualOutput == expectedOutput);
		std::cout << GREEN << "   " << "\u2714" << " Test passed for Intern makeform ShrubberyCreationForm\n" << RESET;
	} catch (std::exception &e) {
		assert(false && "should not thrown an exception");
	}
}

void testInternmakeformNotFounded()
{
	std::stringstream buffer;
    std::streambuf* oldCout = std::cout.rdbuf(buffer.rdbuf());
	...
	try {
		...
		std::string expectedOutput = "Form Not Founded\n";
    	std::cout.rdbuf(oldCout);

		assert(e.what() == expectedOutput);
		std::cout << GREEN << "   " << "\u2714" << " Test passed for Intern makeform Not Founded\n" << RESET;
	}
}
...
int main(void) {
		...
		std::cout << "\n\nAForm Execute Test\n\n";
	{
		...
		std::cout << GREEN << "\n\n<< Intern Test >>\n\n" << RESET;
		std::cout << "\n\nIntern Constructor Call Test\n\n";
	{
		std::cout << "\n[Test3.1.1] Intern Constructor Test\n";
		testInternDefaultConstructor();
		std::cout << "\n[Test3.1.2] Intern Copy Constructor Test\n";
		testInternCopyConstructor();
		std::cout << "\n[Test3.1.3] Intern Assignment Operator Test\n";
		testInternAssignmentOperator();
	}
		std::cout << "\n\nIntern Makeform Test\n\n";
	{
		std::cout << "\n[Test3.2.1] Intern Makeform Presidential Pardon Test\n";
		testInternmakeformPresidentialPardonForm();
		std::cout << "\n[Test3.2.1] Intern Makeform Robotomy Request Test\n";
		testInternmakeformRobotomyRequestForm();
		std::cout << "\n[Test3.2.3] Intern Makeform Shrubbery Creation Test\n";
		testInternmakeformShrubberyCreationForm();
		std::cout << "\n[Test3.2.4] Intern Makeform Not Founded Test\n";
		testInternmakeformNotFounded();
	}
	return 0;
}